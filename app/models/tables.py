...
class User(Base):
    ...
    # be. Encrypted at rest — see app.services.crypto.EncryptedString.
    ...
    totp_pending_secret: Mapped[str | None] = mapped_column(EncryptedString, nullable=True)
...
class SurveyTask(Base):
    ...
    land_use: Mapped[LandUseType | None] = mapped_column(_enum(LandUseType, "land_use_type"), nullable=True)
    boundary_condition: Mapped[BoundaryCondition | None] = mapped_column(
        _enum(BoundaryCondition, "boundary_condition"), nullable=True
    )
    # A short list of tags ("building", "road", "water_body", "trees_crops",
    # "utility", "other") — free-form JSON rather than a join table, since
    # nothing ever queries "every survey that saw a water body", only
    # displays the list back on the one task it belongs to.
    physical_features: Mapped[list | None] = mapped_column(JSONB, nullable=True)
    # A snapshot of which checklist items were satisfied at submit time.
    # Mostly derived from the task's own other fields when the wizard
    # renders it (see api/survey.js on the frontend) rather than a set of
    # independent booleans, so the checklist can never disagree with the
    # data it is supposed to summarise.
    checklist: Mapped[dict | None] = mapped_column(JSONB, nullable=True)

    # --- On-site person, when different from the parcel's recorded owner ---
    # The registered owner (Parcel.owner_id -> Person) is shown to the
    # officer from existing data; these two columns exist only for the case
    # where whoever is actually present and spoken to is someone else — a
    # tenant, an heir, a family member — which this prototype's thin Person
    # model has no field for. Left null when the recorded owner was
    # confirmed as the person on site.
    on_site_person_name: Mapped[str | None] = mapped_column(String(120), nullable=True)
    on_site_person_relation: Mapped[str | None] = mapped_column(String(80), nullable=True)
    person_verified: Mapped[bool] = mapped_column(Boolean, nullable=False, default=False)
    person_verification_note: Mapped[str | None] = mapped_column(Text, nullable=True)
...
class SurveyPhoto(Base):
    ...
    # What the photo shows, e.g. "boundary" or "survey_marker" — nullable so
    # every photo taken before this column existed still reads back fine as
    # unclassified rather than breaking.
    category: Mapped[SurveyPhotoCategory | None] = mapped_column(
        _enum(SurveyPhotoCategory, "survey_photo_category"), nullable=True
    )
...
class Document(Base):
    ...
    id: Mapped[int] = mapped_column(Integer, primary_key=True)
    case_id: Mapped[int] = mapped_column(ForeignKey("cases.id"), nullable=False, index=True)
    # Optionally the survey task this document was gathered for — a survey
    # report or field sketch belongs to one specific piece of fieldwork, not
    # just the case in general. Nullable: every existing case-lifecycle
    # document (a gazette publication, an award copy) has no survey task at
    # all, and version/supersession still works exactly as before since it
    # is keyed on (case_id, doc_type) regardless of this column.
    survey_task_id: Mapped[int | None] = mapped_column(
        ForeignKey("survey_tasks.id"), nullable=True, index=True
    )
    doc_type: Mapped[DocType] = mapped_column(_enum(DocType, "doc_type"), nullable=False)
...
class SurveyDiscrepancy(Base):
    ...
    match the record — mirrors Objection's shape deliberately (same
    case_id/description/status/filed/responded pattern), but is raised by
    the officer doing the survey rather than the affected person, and never
    lets the officer edit the authoritative record directly: it only ever
    creates something for a reviewing SLAO/District Officer to look at."""

    __tablename__ = "survey_discrepancies"

    id: Mapped[int] = mapped_column(Integer, primary_key=True)
    survey_task_id: Mapped[int] = mapped_column(ForeignKey("survey_tasks.id"), nullable=False, index=True)
    # Denormalized, same as Objection.case_id — lets a case-scoped list read
    # every discrepancy on a case without joining through survey_tasks.
    case_id: Mapped[int] = mapped_column(ForeignKey("cases.id"), nullable=False, index=True)
    discrepancy_type: Mapped[DiscrepancyType] = mapped_column(
        _enum(DiscrepancyType, "discrepancy_type"), nullable=False
    )
    description: Mapped[str] = mapped_column(Text, nullable=False)
    status: Mapped[DiscrepancyStatus] = mapped_column(
        _enum(DiscrepancyStatus, "discrepancy_status"), nullable=False, default=DiscrepancyStatus.OPEN
    )
...
class BiometricCredential(Base):
    ...
    # Encrypted at rest — see app.services.crypto.EncryptedString.
    template: Mapped[str] = mapped_column(EncryptedString, nullable=False)
