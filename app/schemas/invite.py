...
Note what is absent: no response model here carries the code itself, except
`InviteCodeIssued`, which exists precisely to hand it over once at creation.
There is no shape in which a stored code can be read back.
"""
...
class InviteCodeOut(BaseModel):
    """An invitation as an administrator sees it afterwards — metadata only."""
